/***********************************************************************
 *  PROYECTO ESP32: CONTROL DE TEMPERATURA (PID + DIMMER) + CNC 2 EJES
 *  - Bloque 1: Control de temperatura con NTC, PID y dimmer de foco
 *  - Bloque 2: CNC 2 ejes (X, Y) con FastAccelStepper + ventilador PWM
 *  - Comunicación: Monitor Serie @115200 (comandos tipo consola)
 *
 *  NOTAS:
 *    - HOME (0,0) está separado ~8 mm de los endstops (no quedan pisados).
 *    - PID SOLO funciona cuando la CNC está homed y en HOME (0,0).
 *    - GPIO2 (LED interno) indica estado del PID:
 *         LOW = PID desactivado / bloqueado
 *         HIGH = PID activo y controlando
 *    - RELAY_LED_PIN controla luces LED solo durante "analisis".
 ***********************************************************************/
...
/* --------------------------------------------------------------------
 *  1. HELPERS DE SALIDA (TODO POR Serial)
 * ------------------------------------------------------------------*/
...
/* --------------------------------------------------------------------
 *  2. CONFIGURACIÓN RÁPIDA (CAMBIAR AQUÍ SEGÚN HARDWARE)
 * ------------------------------------------------------------------*/

// === Frecuencia de red para dimmer ===
...
// Ganancias PID (PID_v1, Ts = 0.1 s)
double Kp = 1.3450;
double Ki = 1.0849;
double Kd = 0.9846;
const uint16_t PID_SAMPLE_MS = 100;   // 100 ms

// Límites de salida PID -> % potencia del foco
const double U_MIN = 25.0;   // 25 %
const double U_MAX = 100.0;  // 100 %

// === Ventiladores de la planta térmica (PWM por LEDC) ===
...
// === Dimmer (foco) ===
const int PIN_ZC    = 27;  // Zero-cross
const int PIN_DIM   = 14;  // Disparo triac
...
// === Relé para luces LED de análisis ===
const int RELAY_LED_PIN = 4;

// === LED interno (GPIO2) indicador de PID activo ===
const int LED_PID_PIN = 2;
...
// === Ventilador PWM CNC (pin directo GPIO con timer ISR) ===
#define FAN_PWM_PIN    17

// === Límites y mecánica CNC ===
...
// Backoff (para aproximación fina) en torno a los endstops
const float X_HOME_BACKOFF_MM = 8.0f;
const float Y_HOME_BACKOFF_MM = 8.0f;

// OFFSET de HOME (0,0) respecto al endstop (zona de parqueo segura)
const float X_HOME_OFFSET_MM  = 8.0f;
const float Y_HOME_OFFSET_MM  = 8.0f;
...
// === Direcciones de los motores (invertir si van al revés) ===
const bool X_DIR_INVERTED = true;
const bool Y_DIR_INVERTED = false;

// === Puntos de análisis (CNC) ===
...
/* --------------------------------------------------------------------
 *  3. BLOQUE TEMPERATURA + PID + DIMMER + VENTILADORES
 * ------------------------------------------------------------------*/
...
// --- Variables globales de temperatura/PID ---
volatile double y_tempC = 25.0;  // Temperatura filtrada
volatile double u_duty  = 0.0;   // Salida PID (%)
...
// --- Aplicar cadena completa de filtros ---
...
inline uint32_t dutyToDelayUs(double dutyPct) {
  ...
  // Márgenes para no disparar demasiado cerca del cruce
  if (delayUs < 200) delayUs = 200;
  if (delayUs > HALF_CYCLE_US - 200) delayUs = HALF_CYCLE_US - 200;
  return delayUs;
}

// Relación inversa entre heater y ventiladores
...
// Suavizado de comando de ventilador
...
/* --------------------------------------------------------------------
 *  4. BLOQUE CNC: STEPPERS, HOMING, MOVIMIENTOS, FAN CNC
 * ------------------------------------------------------------------*/
...
// RTOS EventGroup para estados de homing y movimiento
...
// PID solo activo cuando la CNC está homed y en (0,0)
bool isAtHomePos() {
...
/* ----------------- LIMITES Y HOMING CNC ----------------------------*/

static bool checkLimitX(float mmTarget) {
  EventBits_t b = xEventGroupGetBits(eg);
  if (!(b & EV_X_HOMED)) return true; // antes de homing, permitir
...
// Despegar del endstop antes de homing fino (por si arranca ya pisado)
...
// Rutina genérica de homing por eje con OFFSET final de HOME
                       ...
                       float steps_per_mm)
{
  ...
  // Ahora estamos exactamente en el endstop (cero físico interno).
  // Aplicar OFFSET hacia dentro de la zona útil para que HOME (0,0) quede separado.
  if (home_offset_mm != 0.0f) {
    int inside_dir = homeTowardNeg ? +1 : -1; // dentro de la zona útil
    long st_offset = lroundf(home_offset_mm * steps_per_mm) * inside_dir;
    s->move(st_offset);
    waitStepper(s);
  }

  // La posición lógica 0 se define en la posición de parqueo (despegada del endstop)
  s->setCurrentPosition(0);
  s->setSpeedInHz(speed_normal_hz);
  s->setAcceleration(accel_normal);
  OUT_println("  OK: HOME lógico (0) separado del endstop.");
  return true;
}

// Movimientos
static bool moveToXYAbsSync(float x_mm, float y_mm) {
  if (!checkLimitX(x_mm) || !checkLimitY(y_mm)) {
    OUT_println("  ! Limite excedido");
    return false;
  }
...
/* --------------------------------------------------------------------
 *  5. INTERFAZ DE COMANDOS (PARSER SERIAL)
 * ------------------------------------------------------------------*/
...
/* --------------------------------------------------------------------
 *  6. TAREAS FreeRTOS: SENSE, PID, ACTUADORES, UI, UTIL, MOTION
 * ------------------------------------------------------------------*/

// --- Histeresis para estado sensor_ok ---
...
/* ---------------- Tarea: lectura de temperatura -------------------*/
void taskTemperatureSense(void* pv) {
  for (;;) {
...
/* ---------------- Tarea: PID + dimmer (solo en HOME) --------------*/
void taskPIDControl(void* pv) {
  ...
  for (;;) {
    ...
    // PID SOLO FUNCIONA EN HOME (0,0) y con sensor OK y sin safety_trip
    if (safety_trip || !sensor_ok || !atHome) {
      ...
      // LED interno PID OFF
      digitalWrite(LED_PID_PIN, LOW);
      continue;
    }

    // Condiciones correctas -> activar PID
    ...
    // LED interno PID ON
    digitalWrite(LED_PID_PIN, HIGH);
  }
}

/* ---------------- Tarea: actuadores térmicos (fans IN/OUT) --------*/
void taskHeaterActuators(void* pv) {
  for (;;) {
    double fanPct;

    if (safety_trip || !sensor_ok) {
      fanPct = FAN_MAX_PCT;  // Falla -> ventiladores al máximo
    } else {
...
/* ---------------- Tarea: interfaz de usuario (Serial) --------------*/
void taskUserInterface(void* pv) {
...
/* ---------------- Tarea: utilidades/telemetría (placeholder) ------*/
void taskSystemMonitor(void* pv) {
  TickType_t last = xTaskGetTickCount();
  for (;;) {
    vTaskDelayUntil(&last, pdMS_TO_TICKS(250));
    // Aquí podrías enviar estados por Serial, etc.
  }
}

/* ---------------- Rutinas de homing y análisis (usadas en motion) --*/
...
static void homeX() {
  ...
  bool ok = doHomeAxis(stepperX, endstopXPressed, X_HOME_TOWARD_NEG,
                       X_HOME_BACKOFF_MM, X_HOME_OFFSET_MM,
                       X_HOME_FAST_HZ, X_HOME_SLOW_HZ,
                       X_SPEED_HZ, X_ACCEL, X_STEPS_PER_MM);

  if (ok) xEventGroupSetBits(eg, EV_X_HOMED);
  else    xEventGroupClearBits(eg, EV_X_HOMED);
...
static void homeY() {
  ...
  bool ok = doHomeAxis(stepperY, endstopYPressed, Y_HOME_TOWARD_NEG,
                       Y_HOME_BACKOFF_MM, Y_HOME_OFFSET_MM,
                       Y_HOME_FAST_HZ, Y_HOME_SLOW_HZ,
                       Y_SPEED_HZ, Y_ACCEL, Y_STEPS_PER_MM);

  if (ok) xEventGroupSetBits(eg, EV_Y_HOMED);
  else    xEventGroupClearBits(eg, EV_Y_HOMED);
...
static void go0X() {
  setBusy(true);
  (void)moveToXYAbsSync(0.0f, getYmm());
  setBusy(false);
  OUT_println("OK X=0");
}
...
static void go0Y() {
  setBusy(true);
  (void)moveToXYAbsSync(getXmm(), 0.0f);
  setBusy(false);
  OUT_println("OK Y=0");
}

// --- ANÁLISIS COMPLETO: homing + puntos P1..P6 + volver a (0,0) ---
// Además: RELÉ LED encendido solo durante este recorrido
static void doAnalisis() {
  setBusy(true);

  // Encender relé de luces LED durante análisis
  ...
  // Al terminar el análisis, regresar SIEMPRE a (0,0)
  OUT_println("Regresando a HOME (0,0)...");
  (void)moveToXYAbsSync(0.0f, 0.0f);
  OUT_println("ANALISIS listo. Posicionado en HOME (0,0).");

  // Apagar relé de luces LED (PID vuelve a actuar normal en HOME)
  digitalWrite(RELAY_LED_PIN, LOW);

  setBusy(false);
}

// Ir a un punto P1..P6 (si no está homed, hace homing primero)
static void gotoPoint(uint8_t idx) {
  ...
  char msg[32];
  snprintf(msg, sizeof(msg), "→ Ir a P%u", idx);
  OUT_println(msg);
...
/* ---------------- Tarea: motion planner (procesa cola de comandos) -*/
void taskMotionPlanner(void* pv) {
  Command c;
  for (;;) {
    if (xQueueReceive(qCmd, &c, portMAX_DELAY) == pdTRUE) {
      switch (c.type) {
        ...
        case CMD_FAN_POWER: {
          ...
          fanSetPower(p);
          char msg[32];
          snprintf(msg, sizeof(msg), "Fan17 PWM = %u", p);
          OUT_println(msg);
        } break;
        ...
        case CMD_G0XY:
          setBusy(true);
          (void)moveToXYAbsSync(0, 0);
          setBusy(false);
          OUT_println("OK X=0,Y=0");
          break;

        // --- Puntos P1..P6 ---
        case CMD_P1: case CMD_P2: case CMD_P3:
        case CMD_P4: case CMD_P5: case CMD_P6:
          gotoPoint((uint8_t)(c.type - CMD_P1 + 1));
          break;

        // --- Movimientos relativos en mm ---
        case CMD_REL_MMX:
          setBusy(true);
          if (!moveRelX(c.value)) OUT_println("! Limite X");
          setBusy(false);
          break;

        case CMD_REL_MMY:
          setBusy(true);
          if (!moveRelY(c.value)) OUT_println("! Limite Y");
          setBusy(false);
          break;
...
/* --------------------------------------------------------------------
 *  7. SETUP: INICIALIZAR TODO (HOMING PRIMERO, LUEGO TAREAS)
 * ------------------------------------------------------------------*/
...
void setup() {
  ...
  // --- PWM de ventiladores (IN/OUT) por LEDC ---
  ...
  // --- Pin relé de luces LED ---
  pinMode(RELAY_LED_PIN, OUTPUT);
  digitalWrite(RELAY_LED_PIN, LOW);   // Relé apagado al inicio

  // --- LED interno PID ---
  pinMode(LED_PID_PIN, OUTPUT);
  digitalWrite(LED_PID_PIN, LOW);     // PID OFF al inicio
  ...
  // --- Ventilador CNC PWM por timer ISR ---
  pinMode(FAN_PWM_PIN, OUTPUT);
  GPIO.out_w1tc = (1UL << FAN_PWM_PIN);     // Inicialmente en LOW
  ...
  // --- Infraestructura RTOS ---
  eg   = xEventGroupCreate();
  qCmd = xQueueCreate(16, sizeof(Command));

  // --- Auto-homing al iniciar (motores referenciados primero) ---
  OUT_println("\n=== Auto-homing al iniciar ===");
  homeX();
  homeY();
  moveToXYAbsSync(0.0f, 0.0f);
  OUT_println("Posicionado en HOME (0,0) separado de los endstops.");

  // --- Crear tareas (PID arranca luego de que la mecánica ya está referenciada) ---
  xTaskCreatePinnedToCore(taskTemperatureSense, "sense",   4096, NULL, 2, NULL, 1);
  xTaskCreatePinnedToCore(taskPIDControl,       "control", 4096, NULL, 3, NULL, 1);
  xTaskCreatePinnedToCore(taskHeaterActuators,  "act",     4096, NULL, 1, NULL, 0);

  xTaskCreatePinnedToCore(taskUserInterface,    "UI",      4096, nullptr, 2, &hUI,     0);
  xTaskCreatePinnedToCore(taskSystemMonitor,    "UTIL",    2048, nullptr, 1, &hUtil,   0);
  xTaskCreatePinnedToCore(taskMotionPlanner,    "MOTION",  4096, nullptr, 3, &hMotion, 1);

  OUT_println("Sistema listo. Usa 'help' en el Monitor Serie (115200).");
}

/* --------------------------------------------------------------------
 *  8. LOOP: SOLO TELEMETRÍA SUAVE (NO BLOQUEANTE)
 * ------------------------------------------------------------------*/

void loop() {
  // Imprimir solo la temperatura filtrada cada 5 s
  static uint32_t lastPrint = 0;
  if (millis() - lastPrint >= 5000) {
    lastPrint = millis();
    Serial.println(y_tempC, 2);
  }
