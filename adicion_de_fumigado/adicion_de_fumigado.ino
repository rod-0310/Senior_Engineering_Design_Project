...
void taskControl(void* pv) {
  ...
  for (;;) {
    ...
    // PID SOLO FUNCIONA EN HOME (0,0) y SIN EMERGENCIA
...
void taskActuators(void* pv) {
  for (;;) {
    ...
    if (emergency_stop) {
      fanPct = 0.0;           // TODO APAGADO en emergencia
...
static TaskHandle_t hAutoFumigado = nullptr;   // <<< NUEVO: fumigado automático
...
static void vTaskUI(void*){
  OUT_println("ESP32 listo. Usa 'help' para ver comandos (PID + MOTORES + LED + BOMBA + EMERGENCIA).");
  for(;;){
...
static void doFumigado(){
  ...
  for (uint8_t i=0;i<6 && !emergency_stop;i++){
    ...
    // Ya estamos en el punto: encender bomba 0.3 s, interrumpible
    pumpOn();
    const uint32_t wait_ms = 300;
    for (uint32_t t=0; t<wait_ms && !emergency_stop; t+=50){
      vTaskDelay(pdMS_TO_TICKS(50));
    }
    pumpOff();
  ...
  for(;;){
    if (xQueueReceive(qCmd, &c, portMAX_DELAY) == pdTRUE){
      ...
      switch(c.type){
        ...
        case CMD_FUMIGADO:   doFumigado();   break;

        case CMD_ESTOP:
          // Ya se aplicó en el parser, aquí solo avisamos
        ...
        case CMD_PID_SP_DEC:
          ...
          break;
        case CMD_PID_SHOW:
          Serial.printf("PID: Kp=%.4f  Ki=%.4f  Kd=%.4f | SP=%.2f °C | T=%.2f °C | U=%.2f%% | gate=%d | emerg=%d\n",
...
// <<< NUEVO: periodo de fumigado automático
...
static void vTaskAutoFumigado(void*){
  ...
  for(;;){
    ...
    // Espera a que no haya movimientos en curso
    while (xEventGroupGetBits(eg) & EV_MOTION_BUSY) {
      vTaskDelay(pdMS_TO_TICKS(1000)); // espera 1 s
    }

    // Si no hay emergencia, lanza un fumigado completo
    if (!emergency_stop) {
      uiSend(CMD_FUMIGADO);
    }
  }
...
static void vTaskMqtt(void* pv) {
  ...
  for (;;) {
    ...
    if (now - lastPub >= 2000) {  // cada 2 s
      ...
      char safetyBuf[32];
      snprintf(safetyBuf, sizeof(safetyBuf), "trip=%d,sens=%d,emerg=%d",
               safety_trip ? 1 : 0,
               sensor_ok   ? 1 : 0,
               emergency_stop ? 1 : 0);
...
void setup() {
  ...
  xTaskCreatePinnedToCore(vTaskAutoFumigado,  "AUTO_FUMI",  4096, nullptr, 1, &hAutoFumigado, 0); // <<< NUEVO
