...
volatile bool analysis_running = false;   // <-- NUEVO: indica que está corriendo ANALISIS
...
/* =====================================================================
 *  BLOQUE 2: CNC 2 EJES + VENTILADOR PWM (PIN 17) + RELÉ LUCES
 * ===================================================================== */
...
// Relé para luces LED (elige un pin libre, por ejemplo GPIO 25)
#define PIN_RELAY_LED  22   // activo en HIGH
...
/* ========= SOLO CONTROL PID CUANDO LA CNC ESTÁ EN HOME (0,0) Y SIN ANALISIS ========= */
bool isAtHomePos() {
...
/* ===================== CONTROL RELÉ LUCES ===================== */
inline void relayOn()  { digitalWrite(PIN_RELAY_LED, HIGH); }
inline void relayOff() { digitalWrite(PIN_RELAY_LED, LOW); }
...
void taskControl(void* pv) {
  ...
  for (;;) {
    ...
    // PID SOLO FUNCIONA EN HOME (0,0) Y CUANDO NO HAY ANALISIS
    if (safety_trip || !sensor_ok || !atHome || analysis_running) {
      gate_enable = false;
...
static void doAnalisis(){
  ...
  // Marca que estamos en ANALISIS y enciende las luces
  ...
  OUT_println("ANALISIS listo. Regresando a HOME (0,0)...");
  (void)moveToXYAbsSync(0.0f, 0.0f);

  // Termina ANALISIS: apagar luces y permitir de nuevo PID (cuando esté en HOME)
  analysis_running = false;
  relayOff();

  setBusy(false);
  OUT_println("En HOME (0,0). PID puede seguir trabajando.");
}
...
void setup() {
  ...
  relayOff();   // luces apagadas al inicio
