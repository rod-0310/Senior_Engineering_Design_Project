 ...
 *  - Bloque 2: CNC 2 ejes (X, Y) con FastAccelStepper
 ...
 *    - X_EN_PIN / Y_EN_PIN: los drivers solo se habilitan durante movimiento.
 ***********************************************************************/
...
/* --------------------------------------------------------------------
 *  2. DEFINICIONES DE HARDWARE / CONFIG
 * ------------------------------------------------------------------*/
...
const double U_MIN = 25.0;
const double U_MAX = 100.0;

// Ventiladores planta térmica (PWM LEDC)
...
const int PIN_DIM   = 14;  // Triac gate
...
// Relé de luces LED de análisis
const int RELAY_LED_PIN = 4;

// LED interno indicador de PID
...
// Pines de ENABLE de los drivers (ajusta según tu hardware)
#define X_EN_PIN       25
#define Y_EN_PIN       33
...
// Backoff para aproximación fina
const float X_HOME_BACKOFF_MM = 8.0f;
const float Y_HOME_BACKOFF_MM = 8.0f;

// OFFSET de HOME respecto al endstop (parqueo seguro)
...
/* --------------------------------------------------------------------
 *  3. BLOQUE TEMPERATURA / PID / DIMMER / FANS
 * ------------------------------------------------------------------*/
...
/* --------------------------------------------------------------------
 *  4. BLOQUE CNC (STEPPERS, ENABLE, HOMING)
 * ------------------------------------------------------------------*/
...
// ENABLE de motores (drivers)
inline void motorsEnable(bool on) {
  // Ajusta lógica según tu driver: HIGH = enable típico en TB6600
  digitalWrite(X_EN_PIN, on ? HIGH : LOW);
  digitalWrite(Y_EN_PIN, on ? HIGH : LOW);
}

// PID solo activo cuando CNC está homed y en (0,0)
bool isAtHomePos() {
...
// Separar del endstop por si arranca pisado
                       ...
                       float steps_per_mm)
{
  ...
  // Ahora estamos en el endstop (cero físico)
  if (home_offset_mm != 0.0f) {
    int inside_dir = homeTowardNeg ? +1 : -1;
...
static bool moveToXYAbsSync(float x_mm, float y_mm) {
  ...
  motorsEnable(true); // Habilitar motores para el movimiento
  ...
  motorsEnable(false); // Motores OFF al finalizar
  return true;
}
...
/* --- Tarea: actuadores térmicos (fans planta) --- */
...
/* --- HOMING Y MOVIMIENTOS DE ALTO NIVEL (USADOS POR MOTION) --- */
...
// ANÁLISIS: homing + P1..P6 + volver a (0,0) + relé LED
...
/* --- Tarea: motion planner (lee cola de comandos) --- */
...
void setup() {
  ...
  motorsEnable(false); // motores apagados al inicio
