...
// Credenciales MQTT (como en tu ejemplo del tanque)
...
const char* MQTT_TOPIC_CMD_MODE     = "hidrovida/cmd/mode";  // NUEVO: cambiar modo AUTO/MANUAL
...
void taskControl(void* pv) {
  ...
  for (;;) {
    ...
    // MODO MANUAL: heater al porcentaje manual, sin PID, sin necesidad de estar en HOME
    ...
    // MODO AUTO: PID controla el heater, sólo en HOME
      ...
      // PID SOLO FUNCIONA EN HOME (0,0) y SIN EMERGENCIA / SAFETY
...
void taskActuators(void* pv) {
  for (;;) {
    ...
    if (emergency_stop) {
      ...
      // En modo manual, ventiladores a % manual directamente
...
void applyEmergencyStop() {
  emergency_stop = true;
  safety_trip = true;        // fuerza apagado de heater
  pumpOff();
  ledOff();
  motorsEnable(false);
  gate_enable = false;
...
void clearEmergencyStop() {
  ...
  safety_trip = false;   // liberamos el TRIP para que pueda volver a controlar
}
...
typedef enum : uint8_t {
  ...
  CMD_LED_POWER,
  CMD_ESTOP,          // parada de emergencia
  CMD_ESTOP_CLEAR,    // seguir
  // MODOS
  CMD_MODE_AUTO,
  CMD_MODE_MANUAL,
...
static TaskHandle_t hAutoFumigado = nullptr;   // fumigado automático
...
/* --- Parser ÚNICO (PID + MOTORES + MODOS + MANUAL) --- */
static void parseLineAndDispatch(const String& sIn){
  ...
  // === MOTORES + LED + EMERGENCIA ===
    ...
    // APLICA EMERGENCIA INMEDIATAMENTE
    applyEmergencyStop();
    uiSend(CMD_ESTOP);   // solo para que la tarea de motion imprima mensaje
  }
  else if (s=="seguir") {
    clearEmergencyStop();
...
static void doAnalisis(){
  ...
  // LED de análisis encendido como indicador
  ledOn();
  ...
  // Recorrido por los puntos P1..P6 (solo movimiento, SIN bomba)
  for (uint8_t i=0;i<6 && !emergency_stop;i++){
    ...
    // Tiempo para medición 2s, interrumpible
  ...
  // Al terminar el análisis, regresar a HOME (0,0) si no hay emergencia
  if (!emergency_stop) {
...
static void doFumigado(){
  ...
  // Seguridad: bomba apagada al inicio
  pumpOff();
  ledOn();   // LED como indicador de que está fumigando
  ...
  // Recorrido por los puntos de fumigado F1..F6
  for (uint8_t i=0;i<6 && !emergency_stop;i++){
    char msg[64];
    ...
    // Ya estamos en el punto: encender bomba 0.5 s, interrumpible
    pumpOn();
  ...
  // Al terminar, regresar a HOME (0,0) si no hay emergencia
  if (!emergency_stop) {
    OUT_println("Regresando a HOME (0,0)...");
    (void)moveToXYAbsSync(0.0f, 0.0f);
...
static void vTaskMotion(void*){
  ...
  for(;;){
    if (xQueueReceive(qCmd, &c, portMAX_DELAY) == pdTRUE){
      ...
      // Si hay parada de emergencia, solo aceptamos unos pocos comandos
      if (emergency_stop &&
...
/* ========================= ANALISIS AUTOMÁTICO CADA 1 HORA ========================= */
...
static void vTaskAutoAnalisis(void*){
  ...
  for(;;){
    ...
    // Solo en modo AUTO y sin emergencia
...
/* ========================= FUMIGADO AUTOMÁTICO CADA 10 MIN ========================= */
const uint32_t FUMIGADO_PERIOD_MS = 10UL * 60UL * 1000UL;

static void vTaskAutoFumigado(void*){
  TickType_t last = xTaskGetTickCount();
  const TickType_t period = pdMS_TO_TICKS(FUMIGADO_PERIOD_MS);
...
void reconnectMqtt() {
  while (!mqttClient.connected()) {
    ...
    if (mqttClient.connect(clientId.c_str(), MQTT_USER, MQTT_PASS)) {
      ...
      mqttClient.subscribe(MQTT_TOPIC_CMD_MODE);   // NUEVO: modo AUTO/MANUAL
    } else {
...
void setup() {
  ...
  OUT_println("Sistema listo. Modo inicial: AUTO. Usa 'modo manual' para mantenimiento.");
  OUT_println("También puedes cambiar modo por MQTT en 'hidrovida/cmd/mode' con payload 'auto' o 'manual'.");
}
