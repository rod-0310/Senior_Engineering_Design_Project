...
const bool ENABLE_ACTIVE_LOW = true;   // típico: LOW = enable
...
/* --- Busy + habilitación drivers --- */
static void setBusy(bool on){
  if (on) {
    motorsEnable(true);                 // habilitar drivers cuando hay movimiento
    xEventGroupSetBits(eg, EV_MOTION_BUSY);
  } else {
    // NO deshabilitamos: se quedan energizados en parada
